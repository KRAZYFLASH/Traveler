...
/**
 * Promo Section for Home Page — React + Tailwind
 *
 * Backend contract (example):
 *   GET /api/promotions -> Promotion[]
 *   type Promotion = {
 *     id: string;
 *     title: string;        // "Diskon 20% Kereta ke Bandung"
 *     subtitle?: string;    // "Periode 1–30 Okt"
 *     image?: string;       // hero/banner image URL (optional)
 *     transport?: "flight" | "train" | "bus" | "ship" | "car"; // badge
 *     discountPct?: number; // 10..80
 *     priceFrom?: number;   // 350000
 *     validUntil?: string;  // ISO date
 *     url?: string;         // CTA target
 *     highlight?: boolean;  // show in carousel
 *   }
 */
    ...
    <section className="bg-white" aria-labelledby="promo-heading">
      <div className="mx-auto max-w-6xl px-4 py-8">
              ...
              <div className="absolute inset-0 flex items-center">
                <div className="p-6 text-white md:p-10">
                  ...
                  <a
                    ...
                    className="mt-4 inline-flex rounded-2xl bg-[#FFD60A] px-4 py-2 text-sm font-semibold text-[#1C1C1C] hover:opacity-90"
                  >
                    Ambil Promo
                  </a>
                </div>
        ...
        <div className="mt-6 grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {!loading &&
            bottomCards.map((p) => <PromoCard key={`g-${p.id}`} p={p} />)}
